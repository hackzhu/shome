    ...
    while True:  # 从摄像头读取图片
        ...
        if success:
            ...
            faces = face_cascade.detectMultiScale(
                gray, scaleFactor=1.3, minNeighbors=5, minSize=(int(W_size), int(H_size)))
            ...
            for (x, y, w, h) in faces:
                ...
                T = Total_face_num
                cv2.imwrite(datapath + "/User." + str(T) + '.' +
                            str(sample_num) + '.jpg', gray[y:y + h, x:x + w])

        pictur_num = 10  # 表示摄像头拍摄取样的数量,越多效果越好，但获取以及训练的越慢
...
def Train_new_face():
    ...
    # 调用函数并将数据喂给识别器训练
    faces, ids = get_images_and_labels(datapath)
    ...
    yml = os.path.join(path, str(Total_face_num) + ".yml")
    rec_f = open(yml, "w+")
    rec_f.close()
    recog.save(yml)
...
def get_images_and_labels(path):
    ...
    for image_path in image_paths:
        ...
        # faces = face_cascade.detectMultiScale(img_np) # TODO 查谷歌
...
button_a = tk.Button(window, text='开始刷脸', bg='green', fg='white', font=('Arial', 12), width=10, height=2,
                     command=f_scan_face)
button_a.place(x=600, y=120)

button_b = tk.Button(window, text='录入人脸', bg='green', fg='white', font=('Arial', 12), width=10, height=2,
                     command=f_rec_face)
button_b.place(x=600, y=220)

button_c = tk.Button(window, text='退出', bg='green', fg='white', font=(
    'Arial', 12), width=10, height=2, command=lambda: exit())
