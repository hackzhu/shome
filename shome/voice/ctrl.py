...
dongfile = os.path.join(path, 'resources', 'dong.wav')
recwav = os.path.join(tmpdir, 'rec.wav')
recpcm = os.path.join(tmpdir, 'rec.pcm')
...
wsParam = Ws_Param(APPID='b9b92422', APISecret='NzIxNDBiY2FkZGZlZTk5ZTE3ZmFiMmVm',
                   APIKey='ef0dc0e1d40ef10202c44a7ea3e7333c',
                   AudioFile=recwav)
...
class Wakeup():
    ...
    def callback(self):
        ...
        rec(recwav)
        snowboydecoder.play_audio_file(fname=dongfile)
        # recf = open(recwav,'rb')
        # recf.seek(0)
        # recf.read(44)
        # data = np.fromfile(recf, dtype=np.int16)
        # data.tofile(recpcm)
        # recf.close()
        xunfei()
