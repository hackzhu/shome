import RPi.GPIO as GPIO     # 引入RPI.GPIO模块，并实例化GPIO,简化后面的模块调用
...
class DHT11(object):
    ...
    def collect_input(self):
        ...
        while True:
            ...
            data.append(current)    # 添加current数据
    ...
    # 计算校验值 前四个字节相加取低8位作为校验码(40位，一共5个字节，一个字节8bit)
...
if __name__ == "__main__":
    # 设置引脚及工作方式
    pin_dht = pin_dic['G16']
    GPIO.setmode(GPIO.BOARD)    # 采用BOARD编码方式

    m_DHT11 = DHT11(pin_dht)
    result = m_DHT11.read_DHT()
    while (result[0] == 0):
        result = m_DHT11.read_DHT()
#    print("温度: %-3.1f C\n" % result[1])
    mpub("温度: %-3.1f ℃" % result[1], "homeassistant/temperature")
#    print("湿度: %-3.1f %% \n" % result[2])
    mpub("湿度: %-3.1f %%" % result[2], "homeassistant/humidity")
    result = 0
    time.sleep(2)
    GPIO.cleanup()
